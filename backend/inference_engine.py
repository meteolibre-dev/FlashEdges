...
class FlashEdgesInferenceEngine:
    ...
    def __init__(
        ...
        force_nodata_xpred: bool = True,
        force_metar_xpred: bool = True,
        ...
        Args:
            ...
            force_nodata_xpred: On-manifold forcing at sat no-data pixels.
                In training, a no-data sat pixel (GMGSI off-disk / radar
                outside coverage / elevation nodata) always had x0 = 0, so the
                model only ever saw x_t = t*noise there. At inference nothing
                enforced that: the model's hallucinated x_pred at those pixels
                drove x_t off-manifold, progressively contaminating the
                attention of edge patches spanning both the no-data wedge and
                the valid band (visible as growing artifacts near the +/-73
                deg data edge during denoising). Forcing x_pred = 0 (per
                channel, sat branch only) makes the trajectory EXACTLY
                t*noise -> 0, matching training bit-for-bit at those pixels:
                    v = (x_t - 0)/t  =>  x_{t-dt} = x_t * (t-dt)/t
                METAR non-station pixels are handled separately by
                ``force_metar_xpred`` (hybrid: forced during the trajectory,
                released at the last denoising step).
            force_metar_xpred: Hybrid on-manifold forcing for the METAR branch
                at NON-station pixels (complement of ``metar_station_mask``).
                Training always had x0 = 0 there, so the model only ever saw
                x_t = t*noise at those pixels; at inference the dense
                hallucinated x_pred drives x_t off that marginal
                MID-trajectory, contaminating the shared trunk (attention
                spans sat + metar tokens together -- same mechanism as the
                sat no-data wedge). Fix: force x_pred = 0 at non-station
                pixels for every denoising step EXCEPT THE LAST. x_t then
                follows the exact t*noise -> 0 training trajectory, and at
                the last step (where t_val == dt exactly) the final Euler
                update lands x_t EXACTLY on the model's dense unforced
                x_pred:  x_{t-dt} = x_t - (x_t - x_pred)/t * dt = x_pred.
                The written METAR product therefore keeps the dense
                spatial-fill forecast, generated by ONE clean forward pass
                from an on-manifold state instead of accumulating
                off-manifold drift over the trajectory. Station pixels are
                never forced. Requires ``metar_station_mask`` (always
                provided by run_inference); inactive when
                ``denoising_steps == 1`` (the release step is the only
                step). The AR feedback is unaffected: it re-sparsifies to
                stations regardless. Use --no-force_metar_xpred for A/B.
        ...
        self.force_nodata_xpred = bool(force_nodata_xpred)
        self.force_metar_xpred = bool(force_metar_xpred)
    ...
    def tiled_inference(
        ...
        # On-manifold forcing at sat no-data pixels (force_nodata_xpred):
        # precompute, ONCE, the per-patch per-channel no-data mask (from the
        # LAST context frame, same convention as the yield/feedback paths)
        # for every kept patch. Applying it to x_pred in the denoising loop
        # keeps x_t at no-data pixels on the exact training trajectory
        # (t*noise -> 0) instead of drifting toward hallucinated content.
        forcing_patches = None
        if sat_nodata_mask is not None and self.force_nodata_xpred:
            sat_nodata_last = sat_nodata_mask[:, :, -1:, :, :].bool()
            ps = self.patch_size
            forcing_patches = torch.cat(
                [
                    sat_nodata_last[..., y : y + ps, x : x + ps]
                    for (x, y) in patch_coords
                ],
                dim=0,
            )  # (N_patches, c_sat, 1, ps, ps) bool
            n_forced = int(forcing_patches.any(dim=(1, 2, 3, 4)).sum())
            logger.info(
                f"force_nodata_xpred=True: sat no-data pixels forced to x_pred=0 "
                f"in {n_forced}/{len(patch_coords)} patches (on-manifold "
                f"t*noise decay at every denoising step)"
            )

        # Hybrid METAR forcing (force_metar_xpred): per-patch NON-station
        # mask (~station), broadcast over the 7 METAR channels & forecast
        # frames. Applied to x_pred at every denoising step EXCEPT the last
        # (see __init__ docstring for the t=dt release math).
        metar_forcing_patches = None
        if metar_station_mask is not None and self.force_metar_xpred:
            stn = metar_station_mask.bool()  # (1, 1, 1, H, W)
            ps = self.patch_size
            metar_forcing_patches = torch.cat(
                [
                    ~stn[..., y : y + ps, x : x + ps]
                    for (x, y) in patch_coords
                ],
                dim=0,
            )  # (N_patches, 1, 1, ps, ps) bool, True where NON-station
            logger.info(
                f"force_metar_xpred=True: non-station METAR forced to x_pred=0 "
                f"during denoising steps 0..{self.denoising_steps - 2}; last "
                f"step released (t=dt) so the final x_t lands exactly on the "
                f"dense unforced x_pred (product = one clean forward pass)"
            )
        ...
        while current_step < forecast_steps:
            ...
            for i in tqdm(range(self.denoising_steps), desc="Denoising"):
                ...
                for i_batch in range(0, len(patch_coords), self.batch_size):
                    ...
                    # On-manifold forcing: zero x_pred at sat no-data pixels
                    # (per-channel). With v = (x_t - 0)/t the Euler step gives
                    # x_{t-dt} = x_t*(t-dt)/t, i.e. x_t follows EXACTLY the
                    # t*noise decay training always produced at these pixels.
                    # Applied at EVERY step including the last (the sat
                    # product at no-data pixels is NaN-masked at write time).
                    if forcing_patches is not None:
                        fb = forcing_patches[
                            i_batch : i_batch + len(coords_batch)
                        ]  # (n, c_sat, 1, ps, ps)
                        sat_xp = x_pred_batch[:, :c_sat]
                        x_pred_batch[:, :c_sat] = torch.where(
                            fb.expand_as(sat_xp),
                            torch.zeros_like(sat_xp),
                            sat_xp,
                        )

                    # Hybrid METAR forcing: zero x_pred at NON-station pixels
                    # for every step EXCEPT the last. Mid-trajectory this
                    # keeps x_t on the exact t*noise -> 0 training marginal
                    # (the trunk attends over sat + metar tokens together,
                    # so off-manifold METAR x_t contaminates the sat branch
                    # the same way the sat no-data wedge did). At the last
                    # step the forcing is RELEASED: t_val == dt there, so
                    # the final Euler update lands x_t EXACTLY on the dense
                    # unforced x_pred -- the written METAR product keeps the
                    # spatial-fill forecast from one clean forward pass.
                    # Station pixels are never forced (real x0 there).
                    if (
                        metar_forcing_patches is not None
                        and i < self.denoising_steps - 1
                    ):
                        mb = metar_forcing_patches[
                            i_batch : i_batch + len(coords_batch)
                        ]  # (n, 1, 1, ps, ps)
                        met_xp = x_pred_batch[:, c_sat:]
                        x_pred_batch[:, c_sat:] = torch.where(
                            mb.expand_as(met_xp),
                            torch.zeros_like(met_xp),
                            met_xp,
                        )
